...
public class ImageService 
{
	...
	static
	{
		...
		{
            prop.load(input);
            PATH = prop.getProperty("IMAGE_PATH");
        } 
		catch (IOException e) 
		{
            e.printStackTrace();
            LoggerUtil.logException("Greska u ocitavanju", e);
            throw new RuntimeException("Ne moze se uzitati konfiguracija!");
        }
	}
	//private static final String PATH = "resources" + File.separator + "photos";
	...
	public Image downloadImage(String photoURL)
	{
		...
		{
			LoggerUtil.logException("Greska u kreiranju linka", e);
			e.printStackTrace();
		}
		catch(IOException e)
		{
			LoggerUtil.logException("Greska pri preuzimanju slike", e);
			e.printStackTrace();
		}
		return image;
	}
